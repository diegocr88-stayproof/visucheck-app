...
const PLAN_LIMIT = 20 // vistorias do plano Starter
...
export default function Dashboard() {
  ...
  async function handleAddProperty() {
    ...
    if (!error && newProp) {
      ...
      setModal('none')
      fetchAll()
      // Redireciona para configurar o imóvel
  ...
  const completedThisMonth = inspections.filter(i => {
    ...
    const now = new Date()
    return i.status === 'completed' && date.getMonth() === now.getMonth() && date.getFullYear() === now.getFullYear()
  }).length
  ...
  const s = {
    ...
    card: { background: 'white', borderRadius: '16px', border: '1px solid var(--border)', transition: 'box-shadow 0.2s' } as React.CSSProperties,
    ...
    <div style={s.page}>
      ...
      <div style={s.inner}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '32px' }}>
          <div>
            <h1 style={{ fontFamily: 'Syne, sans-serif', fontSize: '36px', fontWeight: 800, color: 'var(--navy)', letterSpacing: '-1px', marginBottom: '6px' }}>Dashboard</h1>
            <p style={{ fontSize: '15px', color: 'var(--muted)' }}>Gerencie seus imóveis e vistorias</p>
          </div>
          <button style={s.btnGreen} onClick={() => setModal(tab === 'properties' ? 'addProperty' : 'addInspection')}>
            + {tab === 'properties' ? 'Novo Imóvel' : 'Nova Vistoria'}
          </button>
        </div>

        {/* Stats + Plano */}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1.5fr', gap: '16px', marginBottom: '32px' }}>
          {[
          ...
          {/* Saldo do plano */}
          <div style={{ background: 'white', borderRadius: '14px', border: '1px solid var(--border)', padding: '20px 24px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}>
              <div>
                <div style={{ fontSize: '13px', fontWeight: 600, color: 'var(--navy)' }}>Plano Starter</div>
                <div style={{ fontSize: '12px', color: 'var(--muted)', marginTop: '2px' }}>
                  <span style={{ fontFamily: 'Syne, sans-serif', fontSize: '20px', fontWeight: 800, color: planPercent >= 90 ? '#C0392B' : 'var(--navy)' }}>{planUsed}</span>
                  <span style={{ color: 'var(--muted)' }}> / {PLAN_LIMIT} vistorias este mês</span>
                </div>
              </div>
              <div style={{ padding: '4px 10px', borderRadius: '100px', background: planPercent >= 90 ? '#FDECEA' : 'var(--green-glow)', color: planPercent >= 90 ? '#C0392B' : 'var(--green-dark)', fontSize: '12px', fontWeight: 600 }}>
                {PLAN_LIMIT - planUsed} restantes
              </div>
            </div>
            <div style={{ height: '6px', background: 'var(--cream)', borderRadius: '3px', overflow: 'hidden' }}>
              <div style={{ height: '100%', width: `${planPercent}%`, background: planPercent >= 90 ? '#EF4444' : planPercent >= 70 ? '#F59E0B' : 'var(--green)', borderRadius: '3px', transition: 'width 0.5s' }} />
            </div>
          </div>
        </div>
              ...
              <>
                {properties.length === 0 ? (
                  <div style={s.emptyBox}>
                    ...
                    <h3 style={{ fontFamily: 'Syne, sans-serif', fontSize: '22px', fontWeight: 700, color: 'var(--navy)', marginBottom: '8px' }}>Nenhum imóvel cadastrado</h3>
                    <p style={{ fontSize: '15px', color: 'var(--muted)', marginBottom: '32px' }}>Adicione seu primeiro imóvel para começar as vistorias</p>
                    <button style={s.btnGreen} onClick={() => setModal('addProperty')}>+ Adicionar imóvel</button>
                  </div>
                ) : (
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '20px' }}>
                    {properties.map(p => (
                      <div key={p.id} style={{ ...s.card, cursor: 'pointer', overflow: 'hidden' }}
                        onClick={() => navigate(`/property/${p.id}`)}
                        onMouseEnter={e => (e.currentTarget.style.boxShadow = '0 8px 32px rgba(11,45,82,0.12)')}
                        onMouseLeave={e => (e.currentTarget.style.boxShadow = 'none')}
                      >
                        {/* Cover photo */}
                        <div style={{ height: '160px', background: p.cover_url ? 'transparent' : 'linear-gradient(135deg, #e8f0f7, #d0dde9)', position: 'relative', overflow: 'hidden' }}>
                          {p.cover_url ? (
                          ...
                          <div style={{ position: 'absolute', top: '10px', right: '10px', display: 'flex', gap: '6px' }} onClick={e => e.stopPropagation()}>
                            <button style={{ background: 'rgba(255,255,255,0.9)', border: 'none', borderRadius: '8px', padding: '6px 12px', fontSize: '12px', fontWeight: 600, cursor: 'pointer', color: 'var(--navy)' }}
                              onClick={() => navigate(`/property/${p.id}/setup`)}>
                              ⚙️
                            </button>
                            <button style={{ background: 'rgba(46,204,138,0.9)', border: 'none', borderRadius: '8px', padding: '6px 12px', fontSize: '12px', fontWeight: 700, cursor: 'pointer', color: 'var(--navy)' }}
                        ...
                        <div style={{ padding: '16px 20px' }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingTop: '12px', borderTop: '1px solid var(--border)', marginTop: '8px' }}>
                            <span style={{ fontSize: '12px', color: 'var(--muted)' }}>
                              {inspections.filter(i => i.property_id === p.id).length} vistoria(s)
                            </span>
                            <span style={{ fontSize: '12px', color: 'var(--navy)', fontWeight: 600 }}>Ver detalhes →</span>
                          </div>
                        </div>
                      </div>
                    ))}
            ...
            {tab === 'inspections' && (
              <>
                {inspections.length === 0 ? (
                  <div style={s.emptyBox}>
                    <div style={{ fontSize: '48px', marginBottom: '16px' }}>📋</div>
                    <h3 style={{ fontFamily: 'Syne, sans-serif', fontSize: '22px', fontWeight: 700, color: 'var(--navy)', marginBottom: '8px' }}>Nenhuma vistoria realizada</h3>
                    <p style={{ fontSize: '15px', color: 'var(--muted)', marginBottom: '32px' }}>Crie um imóvel e inicie sua primeira vistoria</p>
                    <button style={s.btnGreen} onClick={() => setModal('addInspection')}>+ Nova vistoria</button>
                  </div>
                ) : (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    {inspections.map(insp => {
                      ...
                      return (
                        <div key={insp.id} style={{ ...s.card, display: 'flex', alignItems: 'center', gap: '16px', cursor: 'pointer', overflow: 'hidden' }}
                          onMouseEnter={e => (e.currentTarget.style.boxShadow = '0 8px 32px rgba(11,45,82,0.12)')}
                          onMouseLeave={e => (e.currentTarget.style.boxShadow = 'none')}
                          onClick={() => navigate(`/inspection/${insp.id}/upload`)}
                        >
                          {/* Property cover thumbnail */}
                          <div style={{ width: '72px', height: '72px', flexShrink: 0, background: 'var(--cream)', overflow: 'hidden' }}>
                            {prop?.cover_url ? (
                              <img src={prop.cover_url} alt={prop.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                            ) : (
      ...
      {modal !== 'none' && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 200, background: 'rgba(11,45,82,0.5)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}
          ...
          <div style={{ background: 'white', borderRadius: '20px', padding: '36px', width: '100%', maxWidth: '500px', boxShadow: '0 24px 80px rgba(11,45,82,0.25)' }}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  ...
                  <div>
                    ...
                    {coverPreview ? (
                      ...
                      <button onClick={() => coverInputRef.current?.click()} style={{ width: '100%', height: '120px', borderRadius: '12px', border: '2px dashed var(--border)', background: 'var(--cream)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '8px', cursor: 'pointer' }}
                  ...
                  <div>
                    ...
                    <textarea style={{ ...s.input, resize: 'vertical', minHeight: '72px' }} placeholder="Descrição do imóvel..." value={propDesc} onChange={e => setPropDesc(e.target.value)} />
                  </div>
                  ...
                  <div style={{ padding: '12px 16px', borderRadius: '10px', background: 'var(--green-glow)', border: '1px solid rgba(46,204,138,0.25)', fontSize: '13px', color: 'var(--navy)' }}>
                    📸 Após salvar você será redirecionado para configurar os cômodos e fotos originais
                  </div>

                  <div style={{ display: 'flex', gap: '12px' }}>
                    <button style={{ ...s.btnOutline, flex: 1 }} onClick={() => { setModal('none'); setCoverFile(null); setCoverPreview(null) }}>Cancelar</button>
                    <button style={{ ...s.btnGreen, flex: 2, justifyContent: 'center' }} onClick={handleAddProperty} disabled={saving}>
                      {saving ? 'Salvando...' : 'Salvar e configurar →'}
                    </button>
                  </div>
                </div>
              </>
